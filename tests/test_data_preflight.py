...
def test_committed_plan_is_data_only_and_has_stable_identity():
    ...
    v3, v3_requirements = load_preflight_plan(root / "reports" / "data-field-preflight" / "plan-v3.json")
    ...
    v3_result = json.loads(
        (root / "reports" / "data-field-preflight" / "result-v3.json").read_text(encoding="utf-8")
    )
    recorded_v3_sha = v3_result.pop("result_sha256")
    assert recorded_v3_sha == "91b1331fead7a7392b7a21f406f67e95e57c3ad1fd370f2c0a472c71d276a4dd"
    assert _sha256(v3_result) == recorded_v3_sha
    assert len(v3_result["evidence"]) == 10
    assert sum(item["rows"] for item in v3_result["evidence"]) == 6_055_200
    assert sum(item["quarantined_optional_rows"] for item in v3_result["evidence"]) == 0
